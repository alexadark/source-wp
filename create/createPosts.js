...
module.exports = async ({ actions, graphql }) => {
  ...
  const fetchPosts = async variables =>
    await graphql(GET_POSTS, variables).then(({ data }) => {
      ...
      const blogPagePath = !variables.after ? `/` : `blog/${pageNumber}` //If after is empty, we are on the first page = root of the site if we choose to have the blog has front page
